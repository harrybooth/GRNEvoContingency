...
for exp_name in all_experiments
    ...
    all_networks = reduce(vcat,[et.traversed_networks[2:end] for et in sim[conv]]);
    all_networks_t2s = reduce(vcat,[et.traversed_t2s[2:end] for et in sim[conv]]);
    ...
    sendto(workers(), all_stripe_networks=all_networks)
    sendto(workers(), all_stripe_networks_t2s=all_networks_t2s)

    n_networks = length(all_networks)

    all_networks_dyn_av = pmap(nt->get_av_dyn_vector(nt[1],nt[2],n_steps,n_segments),zip(all_networks,all_networks_t2s));
    ...
    fulld["all_networks_dyn_av"] = all_networks_dyn_av
    fulld["all_networks"] = all_networks
