...
all_experiments = ["DeNovoStripe/AdditionalAnalysis/RE_Minimal_Inhibiting_LongIt"]

@everywhere multi_morph = true

for exp_name in all_experiments

    @everywhere include(srcdirx("ExperimentSetups/" * $exp_name * ".jl"))

    sim = pmap(worker-> SSWM_Evolution_SolverIt(start_network,grn_parameters,Int(1e5),β,max_gen,tolerance,fitness_function,mutate_function),[worker for worker in 1:n_trials])
