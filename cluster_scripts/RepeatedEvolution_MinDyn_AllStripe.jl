...
@everywhere all_experiments = ["Experiment_1/RE_Minimal_Inhibiting_Test"]
...
for exp_name in all_experiments
    ...
    sendto(workers(), all_stripe_networks=all_stripe_networks)
    sendto(workers(), all_stripe_networks_t2s=all_stripe_networks_t2s)

    n_networks = length(all_stripe_networks)

    all_stripe_networks_dyn_cell = pmap(nt->get_rel_dyn_vector(nt[1],nt[2],n_steps,save_id),zip(all_stripe_networks,all_stripe_networks_t2s));
    all_stripe_networks_dyn_av = pmap(nt->get_av_dyn_vector(nt[1],nt[2],n_steps,n_segments),zip(all_stripe_networks,all_stripe_networks_t2s));

    end_X_cell = reduce(hcat,all_stripe_networks_dyn_cell)
    end_X_av = reduce(hcat,all_stripe_networks_dyn_av)
    ...
    min_all_stripe_networks = pmap(n->find_minimal_network(vec(n)[1:10],grn_parameters,DefaultGRNSolver(),fitness_function),all_stripe_networks);
    ...
    fulld["min_all_stripe_networks"] = min_all_stripe_networks
    fulld["all_stripe_network_dict"] = all_stripe_network_dict

    fulld["stripe_achieved"] = stripe_achieved
